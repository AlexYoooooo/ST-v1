...
uint16_t phase(volatile struct Phasor *axis) {
	//Read query
	uint16_t runCtr = (query[axis->phasor+3]<<8) | query[axis->phasor+2];
	uint16_t dirAndDelay = (query[axis->phasor+1]<<8) | query[axis->phasor];
	
	//Enter a loop
	if (dirAndDelay == 0x8000) {
		axis->nestCtr[axis->nestLevel] = runCtr;
		axis->nestLocation [axis->nestLevel++]= axis->phasor + 4;
		/* CurrentCtr points to the beginning of the loop, which indicates the execution count of the loop. */
		/* CurrentCtr + 4 (a instruction word is 4 bytes) points to the first actual instruction of the loop.  */
